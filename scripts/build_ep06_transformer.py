...
"""
    ...
    impl_md(
        ...
        "",
        "| **`GPTConfig`** | fields: `n_vocab`, `n_ctx`, `n_layer`, `n_heads`, `n_hidden`, `mlp_mult` |\n| **`gpt2_small(n_vocab=50257)`** | returns 124M-param GPT-2 Small defaults |\n| **`demo_config(n_vocab)`** | returns tiny config (2 layers, 64 hidden) for notebook demos |",
...
def demo_config(n_vocab: int) -> GPTConfig:
    \"\"\"Small config for notebook demos (fast to init and train).\"\"\"
    return GPTConfig(n_vocab=n_vocab, n_ctx=128, n_layer=2, n_heads=4, n_hidden=64, mlp_mult=4)"""
    ),
    md(
        """## Parameter PyTrees
...
"""
    ...
    impl_md(
        ...
        "_layer_norm",
        "(key: Array, n_hidden: int) -> LayerNormParams",
        "| **`key`** | unused (API symmetry with other inits) |\n| **`n_hidden`** | hidden size `H` |",
    ...
    impl_md(
        ...
        "_mha_params",
        "(key: Array, n_hidden: int) -> MultiHeadAttentionParams",
        "| **`key`** | split into 4 subkeys |\n| **`n_hidden`** | model width `H` |",
    ...
    impl_md(
        ...
        "_mlp_params",
        "(key: Array, n_hidden: int, mlp_mult: int) -> MLPParams",
    ...
    impl_md(
        ...
        "_block_params",
        "(key: Array, n_hidden: int, mlp_mult: int) -> BlockParams",
    ...
    impl_md(
        "### Full model init",
        "Token + position embeddings, `n_layer` blocks, final layer norm.",
        "init_params",
...
"""
    ...
    impl_md(
        ...
        "sample_batch",
        "(key: Array, n_batch: int, n_seq: int, n_vocab: int) -> tuple[Array, Array]",
        "| **`key`** | PRNG key |\n| **`n_batch`** | `B` |\n| **`n_seq`** | `S` |\n| **`n_vocab`** | `V` |",
...
Autoregressive sampling: append one token at a time using `jax.random.categorical` on the last-position logits. Context is truncated to `n_ctx`.
"""
    ),
    impl_md(
...
config = demo_config(n_vocab=enc.n_vocab)
key = jr.key(0)
...
print(f\"gpt2_small would be ~124M params: {gpt2_small(n_vocab=enc.n_vocab)}\")"""
    ),
    md(
...
"""
    ...
    v, s, h, l, m = config.n_vocab, config.n_ctx, config.n_hidden, config.n_layer, config.mlp_mult
    ff = m * h
    per_block = (
